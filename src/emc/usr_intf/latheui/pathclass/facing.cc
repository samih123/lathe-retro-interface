...
void facing_path::create( const tool &tl, vec2 begin, vec2 end )
{
    ...
    while( count >= 0 )
    {
        ...
        create_line( vec2( begin.x + tool_r, z ), MOV_FEED );
        create_line( vec2( end.x + tool_r, z ), MOV_FEED );
        create_line( vec2( end.x + tool_r, z + retract ), MOV_FEED );
        create_line( vec2( begin.x + tool_r, z + retract ), MOV_RAPID );
        --count; 
    }
